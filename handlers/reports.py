...
async def balance_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for line in personal_balance(db, user["id"]):
        card = ju.format_card_number_html(line.other_card) if line.other_card else "شماره کارت ثبت نشده"
        tag = f" (بابت برنامه {line.program_name})" if line.program_name else ""
        if line.i_owe_them:
            lines_owe.append(f"  • به {line.other_name}: {ju.format_money(line.amount)}{tag}  (کارت: {card})")
        else:
            lines_owed.append(f"  • از {line.other_name}: {ju.format_money(line.amount)}{tag}")
...
# ------------------------------------------------------------------ history
def _format_expense_detail(expense: dict) -> str:
    lines = [
    ...
    for p in sorted(expense["participants"], key=lambda x: -x["share_amount"]):
        ...
        lines.append(f"   • {p['first_name']}: {ju.format_money(p['share_amount'])}{payer_tag}")
...
async def _send_history_page(update_or_query, context, offset: int, edit: bool = False):
    ...
    if not expenses:
        ...
        blocks = [_format_expense_detail(e) for e in expenses]
        header = f"🧾 <b>تاریخچه هزینه‌ها</b> ({offset + 1}-{offset + len(expenses)} از {total})\n"
        text = header + "\n\n".join(blocks)
        deletable_ids = [
            e["id"] for e in expenses
            if requester and (requester["is_admin"] or requester["id"] == e["creator_id"])
        ]
        receipt_ids = [e["id"] for e in expenses if e.get("receipt_file_id") or e.get("receipt_text")]
    ...
    if edit:
        ...
        await update_or_query.effective_message.reply_text(text, parse_mode="HTML", reply_markup=markup)
...
async def delete_expense_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if expense is None:
        ...
        return
    if not requester or not (requester["is_admin"] or requester["id"] == expense["creator_id"]):
        await query.answer("فقط مدیر گروه یا کسی که این هزینه رو ثبت کرده می‌تونه حذفش کنه.", show_alert=True)
        return
...
async def view_receipt_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if expense.get("receipt_file_id"):
        await context.bot.send_photo(
            chat_id=query.message.chat_id, photo=expense["receipt_file_id"],
            caption=f"🧾 رسید هزینه #{expense_id} — {expense['description']}",
        )
    elif expense.get("receipt_text"):
        await query.message.reply_text(f"🧾 توضیح رسید هزینه #{expense_id}:\n\n{expense['receipt_text']}")
    else:
...
async def group_report_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not simplified:
        ...
        for frm, to, amt in simplified:
            card = ju.format_card_number_html(to["card_number"]) if to["card_number"] else "بدون شماره کارت"
            lines.append(f"  • {frm['first_name']} ⬅️ {to['first_name']}: {ju.format_money(amt)}  (کارت: {card})")
...
async def export_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    ws1.append(["شماره", "تاریخ", "بابت", "مبلغ کل (تومان)", "پرداخت‌کننده", "نحوه تقسیم", "سهم هرکس", "رسید"])
    for cell in ws1[1]:
        cell.font = Font(bold=True)
    for e in db.list_expenses(limit=1_000_000):
        payer = db.get_user_by_id(e["payer_id"])
        shares_text = "، ".join(f"{p['first_name']}: {p['share_amount']:,}" for p in e["participants"])
        mode_label = "بر اساس تعداد نفرات" if e["split_mode"] == "weighted" else ("مساوی" if e["split_mode"] == "equal" else "مبلغ دلخواه")
        has_receipt = "دارد" if (e.get("receipt_file_id") or e.get("receipt_text")) else "ندارد"
        ws1.append([e["id"], e["jalali_date"], e["description"], e["amount"], payer["first_name"], mode_label, shares_text, has_receipt])
    ...
    for cell in ws2[1]:
        cell.font = Font(bold=True)
    status_fa = {"pending": "در انتظار تایید", "confirmed": "تایید شده", "rejected": "رد شده"}
    for p in db.list_payments(limit=1_000_000):
        frm = db.get_user_by_id(p["from_user_id"])
        to = db.get_user_by_id(p["to_user_id"])
        ws2.append([p["id"], p["jalali_date"], frm["first_name"], to["first_name"], p["amount"],
                    status_fa.get(p["status"], p["status"]), p["note"] or ""])

    ws3 = wb.create_sheet("وضعیت فعلی")
    ws3.sheet_view.rightToLeft = True
    ws3.append(["بدهکار", "طلبکار", "مبلغ (تومان)", "بابت برنامه"])
    for cell in ws3[1]:
        cell.font = Font(bold=True)
    pairs, simplified = group_report(db)
    for debtor, creditor, amt, program_name in pairs:
        ws3.append([debtor["first_name"], creditor["first_name"], amt, program_name or ""])
    ws3.append([])
    ...
    for ws in (ws1, ws2, ws3):
        for col_cells in ws.columns:
            ...
            ws.column_dimensions[col_cells[0].column_letter].width = min(45, max(12, length + 2))

    buffer = io.BytesIO()
    wb.save(buffer)
    buffer.seek(0)
    filename = f"hesabkitab_{ju.jalali_date_str().replace('/', '-')}.xlsx"
    await update.effective_message.reply_document(document=buffer, filename=filename, caption="📊 گزارش کامل حساب‌کتاب گروه")


balance_command_handler = CommandHandler("balance", balance_command)
balance_button_handler = MessageHandler(filters.Regex(f"^{re.escape(kb.BTN_MY_BALANCE)}$"), balance_command)

history_command_handler = CommandHandler("history", history_command)
history_button_handler = MessageHandler(filters.Regex(f"^{re.escape(kb.BTN_HISTORY)}$"), history_command)
history_page_query_handler = CallbackQueryHandler(history_page_callback, pattern=r"^histpage_\d+$")
delete_expense_query_handler = CallbackQueryHandler(delete_expense_callback, pattern=r"^delexp_\d+$")
view_receipt_query_handler = CallbackQueryHandler(view_receipt_callback, pattern=r"^viewreceipt_\d+$")

report_command_handler = CommandHandler("report", group_report_command)
report_button_handler = MessageHandler(filters.Regex(f"^{re.escape(kb.BTN_ADMIN_REPORT)}$"), group_report_command)
