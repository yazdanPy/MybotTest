...
async def balance_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for line in personal_balance(db, user["id"]):
        ...
        if line.i_owe_them:
            lines_owe.append(f"  • به {line.other_name}: {ju.format_money(line.amount)}  (کارت: {card})")
        else:
            lines_owed.append(f"  • از {line.other_name}: {ju.format_money(line.amount)}")
...
async def group_report_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not pairs:
        ...
        for debtor, creditor, amt in pairs:
            lines.append(f"  • {debtor['first_name']} به {creditor['first_name']}: {ju.format_money(amt)}")
