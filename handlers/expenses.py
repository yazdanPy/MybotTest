...
(
    EXP_DESC, EXP_PARTICIPANTS, EXP_SPLIT_MODE, EXP_AMOUNT, EXP_WEIGHTS,
...
async def description_received(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # default: everyone currently selected -- easier to un-check the few who were absent
    ...
    await update.message.reply_text(
        ...
        reply_markup=kb.participants_keyboard(active_users, context.user_data["exp_selected"]),
    )
...
async def participants_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    db = get_db(context)
    active_users = db.list_active_users()
    selected: set = context.user_data.setdefault("exp_selected", {u["id"] for u in active_users})
    ...
    await query.edit_message_reply_markup(reply_markup=kb.participants_keyboard(active_users, selected))
    return EXP_PARTICIPANTS
